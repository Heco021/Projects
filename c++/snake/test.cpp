...
std::mt19937 gen(rd());
int columns = -1, lines = -1, score, frames;
double fps;
...
void inputThread() {
    ...
    while (!stopThread && (ch = getch())) {
        {
            std::unique_lock < std::mutex > lock3(headMutex);
            switch (ch) {
            case 27: {
                std::unique_lock < std::mutex > lock0(stopThreadMutex);
                stopThread.store(true);
...
void generateFood(std::uniform_int_distribution < int > & distributionX, std::uniform_int_distribution < int > & distributionY, std::vector < std::vector < char >> & coordinates) {
    ...
    food.posX = posX;
    food.posY = posY;
    coordinates[food.posY][food.posX] = food.base;
}
int main() {
    ...
    while (!stopThread) {
        {
            std::unique_lock < std::mutex > lock2(coordinatesMutex);
            std::unique_lock < std::mutex > lock3(headMutex);
